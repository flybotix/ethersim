...
public class EtherSimJava
{
  ...
  double dt = 0.001;  // integration step size, seconds
  double tstop = 1.0; // integration duration, seconds
  ...
  void print(){
    ...
    sb.append(t).append(",");
    sb.append(x*3.28083).append(",");
    sb.append(V*3.28083).append(",");
    sb.append(slipping).append(",");
    sb.append(a*3.28083).append(",");
    sb.append(n*A/10).append(",");
    sb.append(Vm);
...
//    printf("%f,%f,%f,%d,%f,%f,%f\n",t,x*3.28083,V*3.28083,slipping,a*3.28083,n*A/10,Vm);
  ...
  void Euler(){ // numerical integration using Euler's Method
    for (t=dt; t<=tstop; t+=dt) {
      V+=a*dt;
      x+=V*dt;
      a = accel(V); 
...
//        build,Kro,Krv,Kf,Vspec,Tspec,Wspec,Ispec,Rcom,Vbat,Rone,n,G,r,M,uk,us); // print CSV header
    ...
    Kt = Tspec/Ispec;
    W = M*9.80665;
    // 0.091 = 0.0762 * (4 * 0.9 * 12.75 * 0.0182)
