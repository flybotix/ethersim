...
public class AppLoader
{
   ...
   // Constants
   public String AppDirName = "SnapCode";
   public String JarName = "SnapCode1.jar";
   public String JarURL = "http://reportmill.com/snap1/SnapCode1.jar.pack.gz";
   public String MainClass = "snap.app.App";
    ...
    * Jar file from local update file, if previously loaded - Load main Jar into
    * URLClassLoader, load main class and invoke main method - Check for update
    * from remove site in background
    */
   public void main1(final String args[]) throws Exception
   {
      ...
      Method meth = cls.getMethod("main", new Class[]
      { String[].class });
    ...
    */
   public void copyDefaultMainJar() throws IOException, ParseException
   {
    ...
    * Check for updates.
    */
   public void checkForUpdatesSilent()
   {
      try
      {
         if(checkForUpdates())
    ...
    * Check for updates.
    */
   public boolean checkForUpdates() throws IOException, MalformedURLException
   {
      boolean result = false;
      // Get URL connection and lastModified time
      File existingJar= getAppFile(JarName);
      if(existingJar.exists())
      {
         URLConnection connection = getConnection();
         long mod0 = existingJar.lastModified(), mod1 = connection.getLastModified();
         result = mod0 < mod1;
         System.out.println("Update available results at " + JarURL + '(' + mod0
                  + '>' + mod1 + ')');         
      }
      else
      {
        System.out.println("App file " + existingJar.getAbsolutePath() + " does not exist");
      }
      return result;
   }
   
   private URLConnection getConnection() throws IOException, MalformedURLException
   {
   ...
   public void getUpdate() throws IOException, MalformedURLException
   {
      ...
      File updatePacked = getAppFile(JarName), updateFile = getAppFile(JarName
            + ".update");
...
//      unpack(updatePacked, updateFile);
      ...
      updateFile.setLastModified(connection.getLastModified());
      updatePacked.delete();
    ...
    */
   private File getAppFile(String aName)
   {
   ...
   /**
    * Returns the file that given packed file would be saved to using the unpack
    * method.
    */
   public static File getUnpackDestination(File aFile, File aDestFile)
   {
      ...
      // If dest file is directory, change to file inside with packed file minus
      // .pack.gz
   ...
   public static byte[] getBytes(InputStream aStream) throws IOException
   {
      ...
      for (int len = aStream.read(chunk, 0, 8192); len > 0; len = aStream.read(
            chunk, 0, 8192))
